...
public class GamePanel extends JPanel implements ActionListener{
	...
	private List<Node> getChildren(Node parent){
		...
		if(x-1>=0 && nodes[x-1][y]==9) {Node child=new Node(x-1,y,9);children.add(child);}
		if(y-1>=0 &&nodes[x][y-1]==9) {Node child=new Node(x,y-1,9);children.add(child);}
		if(x+1<40 &&nodes[x+1][y]==9) {Node child=new Node(x+1,y,9);children.add(child);}
		if(y+1<40&&nodes[x][y+1]==9) {Node child=new Node(x,y+1,9);children.add(child);}
		
		if (x - 1 >=0 && nodes[x-1][y]==0 ) {
			if(nodes[x-1][y]!=9)nodes[x-1][y]=1;
		      Node child = new Node(x - 1, y, nodes[x - 1][y]);
		      children.add(child);
		     
		}
		if (y - 1 >= 0 && nodes[x][y-1]==0) {
			if(nodes[x][y-1]!=9) nodes[x][y-1] = 1;
	...
	protected void paintComponent(Graphics g) {
		...
		for(Map.Entry<Node, Node> set: parents.entrySet()) {
			...
			g.fillRect(SQAURE_SIZE*(set.getKey().getX()), SQAURE_SIZE*(set.getKey().getY()),SQAURE_SIZE, SQAURE_SIZE);
		}
		...
		for(int i=0;i<temp.size();i++) {
			...
			g.fillRect(SQAURE_SIZE*(temp.get(i).getX()), SQAURE_SIZE*(temp.get(i).getY()), SQAURE_SIZE, SQAURE_SIZE);
		}
		...
		for(int i =0; i < obstacles_list.size();i++) {
			g.setColor(Color.black);
			g.fillRect(SQAURE_SIZE*obstacles_list.get(i).getX(), SQAURE_SIZE*obstacles_list.get(i).getY(), SQAURE_SIZE, SQAURE_SIZE);
		}
